  ...
  template: `
    <ion-header>
      <ion-toolbar color="primary">
        ...
        <ion-title>{{ 'SESSIONS.BOOK_NEW' | translate }}</ion-title>
      </ion-toolbar>
    </ion-header>
    ...
    <ion-content class="ion-padding">
      ...
      @if (step() === 'coach') {
        <h3>{{ 'SESSIONS.SELECT_COACH' | translate }}</h3>
        @if (loadingCoaches()) {
          <ion-list>
          ...
          <ion-card>
            <ion-card-content>{{ 'SESSIONS.NO_COACHES' | translate }}</ion-card-content>
          </ion-card>
        } @else {
          <ion-list>
      ...
      @if (step() === 'eventType') {
        <h3>{{ 'SESSIONS.SELECT_TYPE' | translate }}</h3>
        @if (loadingEventTypes()) {
          <div class="spinner-center"><ion-spinner></ion-spinner></div>
        } @else if (eventTypes().length === 0) {
          <ion-card>
            <ion-card-content>{{ 'SESSIONS.NO_EVENT_TYPES' | translate }}</ion-card-content>
          </ion-card>
        } @else {
          <ion-list>
        ...
        @if (loadingSlots()) {
          ...
          <ion-card>
            <ion-card-content>
              ...
              <ion-button fill="outline" (click)="step.set('date')">{{ 'SESSIONS.PICK_ANOTHER_DATE' | translate }}</ion-button>
            </ion-card-content>
          </ion-card>
        } @else {
          <ion-list>
      ...
      @if (step() === 'confirm') {
        <ion-card>
          <ion-card-header>
            <ion-card-title>{{ 'SESSIONS.CONFIRM_BOOKING' | translate }}</ion-card-title>
          </ion-card-header>
          <ion-card-content>
            <p><strong>{{ 'SESSIONS.COACH' | translate }}:</strong> {{ selectedCoach()?.firstName }} {{ selectedCoach()?.lastName }}</p>
            <p><strong>{{ 'SESSIONS.TYPE' | translate }}:</strong> {{ selectedEventType()?.title || selectedEventType()?.name }}</p>
            <p><strong>{{ 'SESSIONS.TIME' | translate }}:</strong> {{ selectedSlot?.label }}</p>
            <p><strong>{{ 'SESSIONS.DURATION' | translate }}:</strong> {{ selectedEventType()?.duration }} min</p>
          </ion-card-content>
        </ion-card>

        <ion-input
          [(ngModel)]="topic"
          [label]="'SESSIONS.TOPIC_OPTIONAL' | translate"
...
export class SessionBookPage implements OnInit {
