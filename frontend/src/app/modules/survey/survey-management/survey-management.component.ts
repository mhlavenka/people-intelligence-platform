...
export class SurveyManagementComponent implements OnInit {
  ...
  intakeTypeTabs = [
    ...
    { key: 'ai_generated',   labelKey: 'SURVEY.showAutoGenerated', icon: 'auto_awesome' },
  ];
  ...
  countByType = (key: string) =>
    key === 'all'
      ? this.templates().length
      : key === 'ai_generated'
      ? this.templates().filter((t) => t.isAutoGenerated).length
      : this.templates().filter((t) => (t.intakeType ?? 'survey') === key && !t.isAutoGenerated).length;
